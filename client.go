...
func (c *Client[TTx]) Insert(ctx context.Context, args JobArgs, opts *InsertOpts) (*rivertype.JobInsertResult, error) {
	...
	res, err := c.insert(ctx, tx, args, opts)
	if err != nil {
		return nil, err
	}
...
func (c *Client[TTx]) InsertTx(ctx context.Context, tx TTx, args JobArgs, opts *InsertOpts) (*rivertype.JobInsertResult, error) {
	res, err := c.insert(ctx, c.driver.UnwrapExecutor(tx), args, opts)
	if err != nil {
		return nil, err
	}
	return res.InsertResults[0], nil
...
func (c *Client[TTx]) insert(ctx context.Context, tx riverdriver.ExecutorTx, args JobArgs, opts *InsertOpts) (*insertManySharedResult, error) {
	res, err := c.validateParamsAndInsertMany(ctx, tx, []InsertManyParams{{Args: args, InsertOpts: opts}})
	if err != nil {
		return nil, err
	}
	return res, nil
...
func (c *Client[TTx]) InsertMany(ctx context.Context, params []InsertManyParams) ([]*rivertype.JobInsertResult, error) {
	...
	res, err := c.validateParamsAndInsertMany(ctx, tx, params)
	if err != nil {
		return nil, err
	}
...
func (c *Client[TTx]) validateParamsAndInsertMany(ctx context.Context, tx riverdriver.ExecutorTx, params []InsertManyParams) (*insertManySharedResult, error) {
...
func (c *Client[TTx]) insertMany(ctx context.Context, tx riverdriver.ExecutorTx, insertParams []*rivertype.JobInsertParams) (*insertManySharedResult, error) {
...
//
// Should only ever be invoke *outside* a transaction. If invoked within a
...
func (c *Client[TTx]) InsertManyFast(ctx context.Context, params []InsertManyParams) (int, error) {
	...
	res, err := c.insertManyFast(ctx, tx, params)
	if err != nil {
		return 0, err
	}
...
func (c *Client[TTx]) notifyProducerWithoutListenerQueueControlEvent(name string, controlEvent *controlEventPayload) {
	if c.driver.SupportsListener() || len(c.producersByQueueName) < 1 {
		return
	}

	if producer, ok := c.producersByQueueName[name]; ok {
		producer.queueControlCh <- controlEvent
	}
}
