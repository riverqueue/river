package example_job_cancel_from_client
...
type SleepingWorker struct {
	river.WorkerDefaults[SleepingArgs]
	jobChan chan int64
}

func (w *SleepingWorker) Work(ctx context.Context, job *river.Job[SleepingArgs]) error {
	w.jobChan <- job.ID
	select {
	case <-ctx.Done():
...
func Example_jobCancelFromClient() {
	...
	insertRes, err := riverClient.Insert(ctx, SleepingArgs{}, nil)
	if err != nil {
		panic(err)
	}
	select {
