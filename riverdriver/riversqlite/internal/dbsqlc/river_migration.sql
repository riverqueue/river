...
-- Unfortunately this doesn't work because of the "table value" pragma isn't
-- supported by sqlc. This seems like it should be fixable though.
-- -- name: ColumnExists :one
-- SELECT EXISTS (
--     SELECT 1
--     FROM /* TEMPLATE: schema */pragma_table_info(@schema_and_table)
