CREATE TABLE river_job (
    ...
    kind VARCHAR(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL,
    max_attempts INT NOT NULL DEFAULT 25,
    metadata JSON NOT NULL DEFAULT (JSON_OBJECT()),
    priority SMALLINT NOT NULL DEFAULT 1,
    queue VARCHAR(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT 'default',
    state VARCHAR(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL DEFAULT 'available',
...
SET
    ...
    finalized_at = CASE WHEN state = 'running' THEN finalized_at ELSE COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')) END,
...
-- name: JobClearUniqueNonce :exec
UPDATE /* TEMPLATE: schema */river_job
SET metadata = JSON_REMOVE(metadata, '$."river:unique_nonce"')
WHERE id IN (sqlc.slice('id'));
...
WHERE
    id IN (
        SELECT id FROM (
            ...
            WHERE (
                (
                    CAST(sqlc.arg('cancelled_do_delete') AS UNSIGNED)
                    AND rj2.state = 'cancelled'
                    AND rj2.finalized_at < sqlc.arg('cancelled_finalized_at_horizon')
                ) OR (
                    CAST(sqlc.arg('completed_do_delete') AS UNSIGNED)
                    AND rj2.state = 'completed'
                    AND rj2.finalized_at < sqlc.arg('completed_finalized_at_horizon')
                ) OR (
                    CAST(sqlc.arg('discarded_do_delete') AS UNSIGNED)
                    AND rj2.state = 'discarded'
                    AND rj2.finalized_at < sqlc.arg('discarded_finalized_at_horizon')
                )
            )
            AND (
                CAST(sqlc.arg('queues_excluded_empty') AS SIGNED)
                OR rj2.queue NOT IN (sqlc.slice('queues_excluded'))
            )
            AND (
                CAST(sqlc.arg('queues_included_empty') AS SIGNED)
                OR rj2.queue IN (sqlc.slice('queues_included'))
            )
            ORDER BY rj2.id
            LIMIT ?
        ) AS tmp
...
WHERE
    ...
    AND scheduled_at <= COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'))
    AND state = 'available'
...
SET
    attempt = attempt + 1,
    attempted_at = COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')),
...
INSERT INTO /* TEMPLATE: schema */river_job(
    ...
    sqlc.narg('id'),
    sqlc.arg('args'),
    COALESCE(sqlc.narg('created_at'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')),
    sqlc.arg('kind'),
    sqlc.arg('max_attempts'),
    CAST(sqlc.arg('metadata') AS JSON),
    sqlc.arg('priority'),
    sqlc.arg('queue'),
    COALESCE(sqlc.narg('scheduled_at'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')),
    sqlc.arg('state'),
    CAST(sqlc.arg('tags') AS JSON),
    sqlc.narg('unique_key'),
    sqlc.narg('unique_states')
) /* TEMPLATE_BEGIN: on_duplicate_key */ ON DUPLICATE KEY UPDATE id = LAST_INSERT_ID(id) /* TEMPLATE_END */;
...
WHERE (sqlc.arg('match') = '' OR LOWER(kind) COLLATE utf8mb4_general_ci LIKE CONCAT('%', LOWER(sqlc.arg('match')), '%'))
    AND (sqlc.arg('after') = '' OR kind > sqlc.arg('after'))
...
SET
    ...
    finalized_at = NULL,
    scheduled_at = COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'))
WHERE id = sqlc.arg('id')
    AND state != 'running'
    AND (
        state <> 'available'
        OR scheduled_at > COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'))
    );
...
WITH eligible AS (
    ...
    WHERE
        ...
        AND river_job.scheduled_at <= COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'))
    ORDER BY
        river_job.priority,
...
SET metadata = JSON_MERGE_PATCH(metadata, '{"unique_key_conflict": "scheduler_discarded"}'),
    finalized_at = COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')),
    state = 'discarded'
WHERE id IN (sqlc.slice('id'));
...
SET
    ...
    finalized_at = CASE WHEN ((sqlc.arg('state') = 'retryable' OR sqlc.arg('state') = 'scheduled') AND JSON_EXTRACT(metadata, '$.cancel_attempted_at') IS NOT NULL)
                    THEN COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'))
