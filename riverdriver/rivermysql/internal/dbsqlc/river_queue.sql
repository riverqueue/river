CREATE TABLE river_queue (
    name VARCHAR(128) CHARACTER SET utf8mb4 COLLATE utf8mb4_bin NOT NULL PRIMARY KEY,
...
INSERT INTO /* TEMPLATE: schema */river_queue (
    ...
    sqlc.arg('name'),
    sqlc.narg('paused_at'),
    COALESCE(sqlc.narg('updated_at'), sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'))
) ON DUPLICATE KEY UPDATE
    updated_at = COALESCE(sqlc.narg('updated_at'), sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00'));
...
SET
    updated_at = CASE WHEN paused_at IS NULL THEN COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')) ELSE updated_at END,
    paused_at = CASE WHEN paused_at IS NULL THEN COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')) ELSE paused_at END;
...
UPDATE /* TEMPLATE: schema */river_queue
SET
    updated_at = CASE WHEN paused_at IS NOT NULL THEN COALESCE(sqlc.narg('now'), CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')) ELSE updated_at END,
    paused_at = NULL;
...
SET
    ...
    updated_at = CONVERT_TZ(NOW(6), @@session.time_zone, '+00:00')
WHERE name = sqlc.arg('name');
