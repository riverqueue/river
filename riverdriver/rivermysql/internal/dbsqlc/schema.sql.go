...
func (q *Queries) ColumnExists(ctx context.Context, db DBTX, arg *ColumnExistsParams) (bool, error) {
	row := db.QueryRowContext(ctx, columnExists, arg.ColumnName, arg.TableName, arg.Schema)
	var exists bool
	err := row.Scan(&exists)
	return exists, err
}
...
func (q *Queries) IndexReindexArtifacts(ctx context.Context, db DBTX, arg *IndexReindexArtifactsParams) ([]string, error) {
	rows, err := db.QueryContext(ctx, indexReindexArtifacts, arg.Schema, arg.ArtifactPattern)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
