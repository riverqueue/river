...
const notificationPollIntervalDefault = 50 * time.Millisecond
...
func (l *Listener) Connect(ctx context.Context) error {
	...
	// Establish the initial high-water mark with a locking scan. InnoDB allocates
	// auto-increment IDs before commit, so a higher ID can commit while a lower
	// ID is still uncommitted. A normal MAX(id) read would advance past the lower
	// row and permanently skip it when it commits. Scan the IDs in ascending
	// order with FOR UPDATE so the read reaches and waits for every lower
	// uncommitted row before establishing the high-water mark. In particular,
	// this must not be reduced to an aggregate MAX or descending LIMIT query,
	// neither of which has to encounter the lower row.
	...
	ids, err := dbsqlc.New().NotificationGetIDsForUpdate(schemaTemplateParam(ctx, schema), notificationDBTX(tx, replacer))
	if err != nil {
		return err
	}
	var lastID int64
...
func (l *Listener) waitForNotificationOnce(ctx context.Context) (*riverdriver.Notification, bool, error) {
	...
	// This must be a locking read in ascending ID order. If a lower ID is still
	// uncommitted while a higher ID has committed, the read waits for the lower
	// row instead of returning the higher row and advancing lastID past it.
