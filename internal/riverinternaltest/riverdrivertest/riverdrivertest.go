...
func Exercise[TTx any](ctx context.Context, t *testing.T,
	...
	t.Run("ColumnExists", func(t *testing.T) {
		...
		// In SQLite, the schema needs to be attached. No-op for Postgres.
		//
		// Notably, the new schema is not detached. I initially tried to do so,
		// but it can't be detached while the test transaction is running (you
		// get an error like "SQL logic error: database
		// river_test_2025_04_30t12_26_17_schema_03 is locked (1)"), and rolling
	...
	t.Run("JobSetStateIfRunningMany_JobSetStateCompleted", func(t *testing.T) {
		...
		t.Run("UnknownJobIgnored", func(t *testing.T) {
			...
			// of an unknown job. This may not have been a particularly well
			// thought out decision, but I've codified here later to help make
			// sure all drivers behave consistently.
	...
	t.Run("TableExists", func(t *testing.T) {
		...
		// In SQLite, the schema needs to be attached to our existing executor.
		// No-op for Postgres.
		//
		// Notably, the new schema is not detached. I initially tried to do so,
		// but it can't be detached while the test transaction is running (you
		...
		// doesn't matter much if an extra schema is attached until the tests
		// finish running.
		// require.NoError(t, exec.SchemaAttach(ctx, &riverdriver.SchemaAttachParams{Schema: schema}))
