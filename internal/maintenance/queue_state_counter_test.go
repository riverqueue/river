...
func TestQueueStateCounter(t *testing.T) {
	...
	setup := func(t *testing.T, hooks []rivertype.Hook, queueNames []string) (*QueueStateCounter, *testBundle) {
		...
		counter.StaggerStartupDisable(true)
		counter.TestSignals.Init(t)
		t.Cleanup(counter.Stop)

		return counter, &testBundle{exec: exec}
	}
	...
	t.Run("CountsJobsByQueueAndState", func(t *testing.T) {
		...
		counter, bundle := setup(t, []rivertype.Hook{hook}, []string{"queue1", "queue2", "queue_empty"})
		...
		require.NoError(t, counter.Start(ctx))
		counter.TestSignals.CountedOnce.WaitOrTimeout()
	...
	t.Run("StartStop", func(t *testing.T) {
		...
		counter, _ := setup(t, []rivertype.Hook{&capturingQueueStateCountHook{}}, nil)
	...
	t.Run("StartStopStress", func(t *testing.T) {
		...
		counter, _ := setup(t, []rivertype.Hook{&capturingQueueStateCountHook{}}, nil)
		counter.Logger = riversharedtest.LoggerWarn(t)       // loop started/stop log is very noisy; suppress
		counter.TestSignals = QueueStateCounterTestSignals{} // deinit so channels don't fill

		startstoptest.Stress(ctx, t, counter)
	})
