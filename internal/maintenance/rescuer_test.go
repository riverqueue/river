...
func TestRescuer(t *testing.T) {
	...
	t.Run("RescuesStuckJobs", func(t *testing.T) {
		...
		stuckToCancelJob1 := insertJob(ctx, bundle.tx, insertJobParams{State: dbsqlc.JobStateRunning, AttemptedAt: ptrutil.Ptr(bundle.rescueHorizon.Add(-1 * time.Hour)), Metadata: []byte(`{"cancelled_by_query": true}`)})
		stuckToCancelJob2 := insertJob(ctx, bundle.tx, insertJobParams{State: dbsqlc.JobStateRunning, AttemptedAt: ptrutil.Ptr(bundle.rescueHorizon.Add(1 * time.Minute)), Metadata: []byte(`{"cancelled_by_query": true}`)}) // won't be rescued
