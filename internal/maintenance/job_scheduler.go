...
func (s *JobScheduler) runOnce(ctx context.Context) (*schedulerRunOnceResult, error) {
	...
	for {
		...
		numScheduled, err := func() (int, error) {
			...
			results, err := s.exec.JobSchedule(ctx, &riverdriver.JobScheduleParams{
				Max: s.config.Limit,
			...
			for _, result := range results {
				if result.ScheduledAt.After(notificationHorizon) {
					continue
				}

				queues = append(queues, result.Queue)
			}
			...
			return len(results), execTx.Commit(ctx)
		}()
		if err != nil {
			return nil, err
		}
