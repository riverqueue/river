...
func TestBatchCompleter_NewerResultSupersedesRequeuedResult(t *testing.T) {
	t.Parallel()
	...
	type testBundle struct {
		...
		// persisted records the state of each job ID in every set-state call
		// that succeeded.
		persisted   []map[int64]rivertype.JobState
		persistedMu sync.Mutex
	}

	setup := func(t *testing.T, failCalls int, beforeFailedCall func(bundle *testBundle, callNum int)) *testBundle {
		t.Helper()

		bundle := &testBundle{execMock: &partialExecutorMock{}}

		var numCalls int
		bundle.execMock.JobSetStateIfRunningManyFunc = func(ctx context.Context, params *riverdriver.JobSetStateIfRunningManyParams) ([]*rivertype.JobRow, error) {
			numCalls++
			if numCalls <= failCalls {
				if beforeFailedCall != nil {
					beforeFailedCall(bundle, numCalls)
				}
				return nil, errors.New("error from batch completion")
			}

			states := make(map[int64]rivertype.JobState, len(params.ID))
			rows := make([]*rivertype.JobRow, len(params.ID))
			for i, id := range params.ID {
				states[id] = params.State[i]
				rows[i] = &rivertype.JobRow{ID: id, State: params.State[i]}
			}

			bundle.persistedMu.Lock()
			bundle.persisted = append(bundle.persisted, states)
			bundle.persistedMu.Unlock()

			return rows, nil
		}

		bundle.completer = NewBatchCompleter(riversharedtest.BaseServiceArchetype(t), "", bundle.execMock, &riverpilot.StandardPilot{}, make(chan []CompleterJobUpdated, 10))
		bundle.completer.disableSleep = true

		return bundle
	}

	setState := func(t *testing.T, completer *BatchCompleter, params *riverdriver.JobSetStateIfRunningParams) {
		t.Helper()

		require.NoError(t, completer.JobSetStateIfRunning(ctx, &jobstats.JobStatistics{}, params))
	}

	t.Run("NewerResultArrivesAfterRequeue", func(t *testing.T) {
		t.Parallel()

		bundle := setup(t, numRetries, nil)

		setState(t, bundle.completer, riverdriver.JobSetStateCompleted(1, time.Now(), nil))
		require.Error(t, bundle.completer.handleBatch(ctx))

		setState(t, bundle.completer, riverdriver.JobSetStateErrorRetryable(1, time.Now(), []byte(`{}`), nil))
		require.NoError(t, bundle.completer.handleBatch(ctx))

		require.Equal(t, []map[int64]rivertype.JobState{{1: rivertype.JobStateRetryable}}, bundle.persisted)
	})

	t.Run("NewerResultArrivesWhileInFlight", func(t *testing.T) {
		t.Parallel()

		// The newer result for job 1 arrives while the batch carrying the
		// older result is failing, so it's deferred behind the in-flight batch.
		bundle := setup(t, numRetries, func(bundle *testBundle, callNum int) {
			if callNum == 1 {
				setState(t, bundle.completer, riverdriver.JobSetStateErrorRetryable(1, time.Now(), []byte(`{}`), nil))
			}
		})
		...
		setState(t, bundle.completer, riverdriver.JobSetStateCompleted(1, time.Now(), nil))
		setState(t, bundle.completer, riverdriver.JobSetStateCompleted(2, time.Now(), nil))
		require.Error(t, bundle.completer.handleBatch(ctx))

		bundle.completer.setStateParamsMu.RLock()
		require.Empty(t, bundle.completer.deferredSetStateParams)
		require.Len(t, bundle.completer.setStateParams, 2)
		bundle.completer.setStateParamsMu.RUnlock()

		require.NoError(t, bundle.completer.handleBatch(ctx))

		require.Equal(t, []map[int64]rivertype.JobState{{
			1: rivertype.JobStateRetryable,
			2: rivertype.JobStateCompleted,
		}}, bundle.persisted)
	})
}
