...
func Test_Client(t *testing.T) {
	...
	t.Run("QueueUpdateMetadata", func(t *testing.T) {
		t.Parallel()
		...
		queue, err = client.QueueUpdateMetadata(ctx, queue.Name, []byte(`{"foo":"bar"}`))
		require.NoError(t, err)
		...
		queue, err = client.QueueUpdateMetadata(ctx, queue.Name, []byte(`{"foo":"baz"}`))
		require.NoError(t, err)
