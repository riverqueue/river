...
var (
	...
	errDBPoolMySQL  error     //nolint:gochecknoglobals
)
...
func DBPoolMySQL(ctx context.Context, tb testing.TB) *sql.DB {
	...
	dbPoolMySQLOnce.Do(func() {
		...
		dbPoolMySQL, errDBPoolMySQL = sql.Open("mysql", dsn)
		if errDBPoolMySQL == nil {
			errDBPoolMySQL = dbPoolMySQL.PingContext(ctx)
		}
	})
	require.NoError(tb, errDBPoolMySQL)
