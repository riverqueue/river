...
func TestWorker_Work(t *testing.T) {
	...
	t.Run("Reusable", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Equal(t, river.EventKindJobCompleted, res.Kind)
		res, err = tw.Work(context.Background(), t, bundle.tx, testArgs{Value: "test2"}, nil)
		require.NoError(t, err)
		require.Equal(t, river.EventKindJobCompleted, res.Kind)
	})
	...
	t.Run("SetsCustomInsertOpts", func(t *testing.T) {
		...
		res, err := tw.Work(context.Background(), t, bundle.tx, testArgs{Value: "test3"}, &river.InsertOpts{
			MaxAttempts: 420,
	...
	t.Run("UniqueOptsAreIgnored", func(t *testing.T) {
		...
		res, err := tw.Work(context.Background(), t, bundle.tx, testArgs{Value: "test"}, &river.InsertOpts{
